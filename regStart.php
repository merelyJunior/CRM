...
include "application/models/model_token.php";
   ...
   function getCount($token) {
    ...
    function sendDialog($token, $count, $country) {
        $url = "https://185.250.148.231/startreg/";
        $postdata = json_encode(array('access'=>$token,'count'=>$count,'country'=>$country));
        $ch = curl_init($url); 
    ...
    // print_r($country);
    $res = sendDialog($token, $count,$country);
    // print_r($time);
    $time = $res->time;
    echo ($time);
